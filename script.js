...
fileInput.addEventListener("change", (event) => {
  ...
  if (file) {
    ...
    readerForApi.onloadend = () => {
      imageBase64 = readerForApi.result.split(",")[1]
    }
...
servingsSelector.addEventListener("change", (event) => {
  if (event.target.name === "servings") {
    currentServings = Number.parseInt(event.target.value, 10)
    if (baseRecipeForOne) {
...
async function analyzeImage(base64ImageData) {
  ...
  const googleScriptUrl =
    "https://script.google.com/macros/s/AKfycbwStTn70XfuiDlrJRCrf4S3i8pJPtI7zuJJFZjuM9FQmgCPOmyq11QCtAJu7I4LGamd/exec"
  ...
  try {
    ...
    console.log("Respuesta del servidor:", resultText) // Agregando log para debugging
    ...
    baseRecipeForOne = resultJson
    updateDisplayForServings()
  } catch (error) {
    console.error("Error detallado:", error)
    showError(`Lo siento, hubo un problema. (${error.message})`)
  } finally {
    analyzeButton.disabled = false
  }
}
...
function generateShareableText() {
  ...
  shareText += `\n📝 *Instrucciones:*\n`
  baseRecipeForOne.instructions.forEach((step, i) => {
    shareText += `${i + 1}. ${step}\n`
  })
